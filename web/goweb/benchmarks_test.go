...
func routerFunc(c *Context) {
}

func loadRouter([]route) http.Handler {
	r := New()

	for _, api := range githubAPI {
		switch api.method {
		case "GET":
			r.GET(api.path, routerFunc)
		case "POST":
			r.POST(api.path, routerFunc)
		case "PUT":
			r.PUT(api.path, routerFunc)
		case "DELETE":
			r.DELETE(api.path, routerFunc)
		}
	}
...
func BenchmarkGoWeb_Param(b *testing.B) {
...
func BenchmarkGoWeb_GithubStatic(b *testing.B) {
	router := loadRouter(githubAPI)
